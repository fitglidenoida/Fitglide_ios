...
class StrapiApiClient: StrapiApi {
    ...
    private func performRequest<T: Codable>(_ request: URLRequest) async throws -> T {
        logger.debug("Sending request: \(request.httpMethod ?? "Unknown") \(request.url?.absoluteString ?? "No URL")")
        ...
        do {
            ...
            logger.debug("Successfully decoded response for type: \(T.self)")
    ...
    private func buildRequest(method: String, path: String, queryItems: [URLQueryItem] = [], body: Codable? = nil, token: String? = nil) throws -> URLRequest {
        ...
        if let token = token {
            ...
            logger.debug("Added Authorization header with token: \(token.prefix(10))...")
        }
        if let body = body {
    ...
    // User Profile
    func getUserProfile(token: String) async throws -> UserProfileResponse {
        let queryItems = [URLQueryItem(name: "populate", value: "*")]
        let request = try buildRequest(method: "GET", path: "users/me", queryItems: queryItems, token: token)
        return try await performRequest(request)
    }
    ...
    func initiateStravaAuth(state: String) async throws -> StravaAuthResponse {
        ...
        let request = try buildRequest(method: "GET", path: "strava-auth", queryItems: queryItems)
        return try await performRequest(request)
    }
